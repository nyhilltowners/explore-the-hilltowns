...
def read_rows(path: Path, where: str):
...
def parse_folklore(path: Path, label: str):
    where = path.name
    hdr, rows = read_rows(path, where)
...
def parse_poi(path: Path, label: str):
    where = path.name
    hdr, rows = read_rows(path, where)
    ...
    for n, row in enumerate(rows, start=2):
        ...
        if not any(hrs.values()):
            warn(f"{rw}: no hours given for any day (fine if not applicable)")
        out.append({
            "ty": "poi", "cat": label, "id": rid, "t": title or "(unnamed)",
            "tags": s(cell(row, idx, "tags")), "addr": s(cell(row, idx, "addr")),
            "lat": lat, "lng": lng, "s": s(cell(row, idx, "stry")),
            "hrs": hrs, "web": web, "ph": s(cell(row, idx, "ph")),
            "g": s(cell(row, idx, "g")), "tier": "exact",
            "img": norm_image(cell(row, idx, "img"), rw),
            "cred": s(cell(row, idx, "cred")),
        })
    check_dupe_ids(out, where)
...
def parse_events(path: Path, label: str):
    where = path.name
    hdr, rows = read_rows(path, where)
...
def main() -> int:
    ...
    for cat in manifest["categories"]:
        ...
        if not wb_path.exists():
            ...
            continue
        recs = PARSERS[schema](wb_path, label)
    ...
    (SITE / "data.js").write_text("window.ATLAS_DATA = " + js + ";\n",
                                  encoding="utf-8")
    shutil.copyfile(ROOT / "index.template.html", SITE / "index.html")
